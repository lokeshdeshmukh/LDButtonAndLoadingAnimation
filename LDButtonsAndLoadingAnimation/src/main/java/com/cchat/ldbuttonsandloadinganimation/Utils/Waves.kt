package com.cchat.ldbuttonsandloadinganimation.Utils
...
import com.cchat.ldbuttonsandloadinganimation.R
...
constructor(context: Context,
    ...
    init {
        val attrs = context.obtainStyledAttributes(attrs, R.styleable.Waves, defStyleAttr, 0)

        //init paint with custom attrs
        wavePaint = Paint(ANTI_ALIAS_FLAG).apply {
            color = attrs.getColor(R.styleable.Waves_waveColor, 0)
            strokeWidth = attrs.getDimension(R.styleable.Waves_waveStrokeWidth, 0f)
            style = Paint.Style.STROKE
        }

        waveGap = attrs.getDimension(R.styleable.Waves_waveGap, 50f)
        attrs.recycle()
    }
